...
import { usePublicationsMarketing } from "../../shared/hooks/Publications/useListPublicationsMarketing";
...
export const ListPublicationsMarketing = () => {
  const { publications, loading, error, refresh } = usePublicationsMarketing();
  ...
  const { handleToggleLike, loading: likeLoading, error: likeError } = useToggleLike();
  ...
  // Actualizar estado de likes cuando cambie user o publications
  useEffect(() => {
  ...
  const handleCommentSubmit = (pubId) => {
    if (!newComments[pubId]?.trim()) return;
    
    // Aquí iría la lógica para enviar el comentario
    console.log("Enviando comentario:", newComments[pubId]);
    setNewComments(prev => ({
      ...prev,
      [pubId]: ""
    }));
  ...
  if (error) {
    return (
      <Box bg={bgPrimary} minH="100vh" py={8}>
        ...
        >
          <VStack spacing={4}>
            <Text color="red.500" textAlign="center" fontSize="lg" fontWeight="600">
              {error}
            </Text>
            <Button 
              colorScheme="red" 
    ...
    <Box bg={bgPrimary} minH="100vh" py={6}>
      ...
      >
        ...
        <ModalContent borderRadius="3xl" p={4} bg={bgBox}>
          ...
          <ModalBody p={0}>
            {selectedPub && (
              <EditPublication publication={selectedPub} onClose={() => {
                setSelectedPub(null);
                onEditClose();
              }} />
            )}
                ...
                <Box p={5} pb={4}>
                  <HStack spacing={4}>
                    ...
                    <Menu>
                      ...
                      />
                      <MenuList>
                        {user?.role === "DEVELOPER_ROLE" || user?.username === pub.author?.username ? (
                          <>
                            <MenuItem
                              onClick={() => {
                                setSelectedPub(pub);
